...
def F1_score(G,S):
    """Returns F1 Score for ground truth and predicted masks."""
...
for images in tqdm(image_names):
    ''' Read Predictions S and Ground Truths G'''
    ...
    ''' Collect Dice Score for each S, G'''
    mean_dice.append(Dice(G, S))
    
''' Calculate the overall(mean) Dice Score'''        
print ('Mean_Dice = ', np.mean(np.array(mean_dice)))

    
''' Store Scores for Dice, F1 and Jaccard'''
f = open('lung.txt','w')
